		      ...
		      long long offset, long long length)
{
    ...
    printk(KERN_ERR "Function call may be wrong, due to gumstix patch");
	new = allocate_partition(master, &part, -1, offset, 0);
	if (IS_ERR(new))
		return PTR_ERR(new);
		       ...
		       int nbparts)
{
	...
	for (i = 0; i < nbparts; i++) {
		slave = allocate_partition(master, parts + i, i, cur_offset, nbparts);
		if (IS_ERR(slave))
